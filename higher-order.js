/*
  ...
  Passed tests will be indicated by a arr circle.
...
// let evens = mixedNumbers.filter(even => even % 2 === 0);
...
//Code Here
  let totalPopulation = 0 //  = populations.reduce(/* Provide Your Callback Here */)
...
let orderTotals = orders.map( arr => arr['price'] += arr['price']*arr['tax']);
...
*/
let bobsTotal = purchases.reduce((total,green) => {
  if(green['owner'] === 'Bob'){
    total += green['price'];
  }
  return total;
